  ...
  async (req, res, next) => {
   ...
   let resSelect=await db.queryAsync(sql,[req.body.username])
    if(resSelect.length!==0){
      ...
      let sql="insert into user(username,photo,password,mobile,email,status,create_time,address，spend) value(?,?,?,?,?,?,?,?,?)"
      let params=[
          req.body.username,
          'images/photo.webp',
          req.body.password,
...
router.post('/login',async (req, res, next) => {
    ...
    console.log(4)
    let sql = "select * from manager where username=? and password=?"
    let params = [
        req.body.username,
        req.body.password
    ]
...
router.post('/addUser',async (req,res)=>{
    ...
    let resSelect=await db.queryAsync(sql,[req.body.username])
    if(resSelect.length!==0){
       return res.send({flag:false,msg:'此业主已经存在'})
    }else{
        let sql="insert into user(username,photo,password,mobile,email,address,status,create_time，cast) value(?,?,?,?,?,?,?,?,?)"
        let params=[
            req.body.username,
            'images/photo.webp',
            req.body.password,
        ...
        let resInsert=await db.queryAsync(sql,params)
        if(resInsert.affectedRows!==0){
           return res.send({flag:true,msg:'添加业主成功'})
        }else{
           return res.send({flag:false,msg:'添加业主失败'})
        }
    }
})
...
router.put('/editUser/:id',async (req, res) => {
    ...
    */
    ...
    if(resUpdate.affectedRows!==0){
      return res.send({flag:true,msg:"修改业主信息成功！"})
    }else{
        return res.send({flag:false,msg:"修改业主信息失败！"})
    }
})

router.put('/editUserCast/:id',async (req, res) => {

    let sql="update user set cast=? where id=?"
    let cast=req.body.cast,id=req.params.id
    let resUpdate=await db.queryAsync(sql,[cast,id])
    //console.log(resUpdate.affectedRows)
    if(resUpdate.affectedRows!==0){
        return res.send({flag:true,msg:"修改业主水电缴费费信息成功！"})
    }else{
        return res.send({flag:false,msg:"修改业主水电缴费信息失败！"})
    }
})
...
router.delete('/delUser/:id',async (req, res) => {
    ...
    if (delResult.affectedRows!==0){
        return res.send({flag:true,msg:'删除业主成功！'})
    }else{
        return res.send({flag:false,msg:'删除业主失败！'})
    }
})
...
router.put('/userStatus/:id/status/:status',(req, res) => {
    ...
    db.query(sql,[status,id],(err,data)=>{
        if (err){
            ...
            if(data.length!==0){
                res.send({flag:true,msg:'修改业主状态成功！'})
            }else{
                res.send({flag:false,msg:'修改业主状态失败！'})
            }
        }
    })
})

router.put('/userSpend/:id/spend/:spend',(req, res) => {
    //console.log(req.params)  //{ id: '2', status: '1' }
    let id=req.params.id,spend=req.params.spend
    let sql="update user set spend=? where id=?"
    db.query(sql,[spend,id],(err,data)=>{
        if (err){
            throw err
        }else{
            if(data.length!==0){
                res.send({flag:true,msg:'修改业主缴费信息成功！'})
            }else{
                res.send({flag:false,msg:'修改业主缴费信息失败！'})
            }
        }
    })
})
